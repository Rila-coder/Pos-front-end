      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground flex items-center gap-2">
            <PackageCheck className="text-primary w-6 h-6 sm:w-8 sm:h-8" />{" "}
            Global Inventory
          </h1>
          <p className="text-xs sm:text-sm text-muted-foreground">
            Managing stock across all business branches
          </p>
        </div>
        <Button
          onClick={() => setOpen(true)}
          className="bg-primary hover:bg-primary-hover font-bold uppercase text-[10px] sm:text-xs shadow-lg shadow-primary/20 h-9 sm:h-11 px-4 sm:px-6"
        >
          <Plus className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" /> Global Add
          Product
        </Button>
      </div>
